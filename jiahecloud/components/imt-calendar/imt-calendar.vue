	...
	<view class="imt-calendar">
		...
		<swiper class="calendar-date-swiper" :current="current" @change="current = $event.detail.current">
			<swiper-item class="calendar-date-wrapper" v-for="(item,index) in calendar" :key="index">
				...
				<!-- <view class="calendar-date" @click="clickF(value.date)" v-for="(value,key) in item" :key="key" :style="{background:value.selected?value.selected&&color:value.chosed?value.chosed&&colored:'#fff',color:value.selected?value.selected&&'#fff':value.chosed&&'#fff'}">{{value.date}}</view> -->
				<view class="calendar-date" @click="clickF(value.date)" v-for="(value,key) in item" :key="key">
					{{value.date}}
					<view class="spot" :style="{background:value.selected?value.selected&&color:value.chosed?value.chosed&&colored:'#fff',color:value.selected?value.selected&&'#fff':value.chosed&&'#fff'}"></view>
				</view>
			</swiper-item>
		</swiper>
	</view>
...
<script>
	export default {
		...
		created() {
			...
			this.chosed.forEach(i => {
				let time = i.split('-')
				this.calendar[time[1] - this.startMonth - 1 + (time[0] - this.startYear) * 12][time[2] - 1 + new Date(
					`${time[0]}-${time[1]}`).getDay()].chosed = true
			})
		...
		methods: {
			...
			clickF(str){
				this.$emit('loadDetail',this.format(this.current)+(str.toString().length===1?('-0'+str):('-'+str)));
			}
		},
		computed: {
			currentTime() {

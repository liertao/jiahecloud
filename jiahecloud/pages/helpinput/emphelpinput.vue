		...
		<view class="example-body">
			<u-navbar>
				<view class="search-wrap" v-if="true">
					...
					<u-search v-model="keyword" placeholder="请输入员工姓名或者编码" :show-action="true" height="56" :action-style="{color: '#fff'}" @change="input" style="width: 100%;"></u-search>
				</view>
			</u-navbar>
			<u-swipe-action btn-width="120" :show="item.show" :index="index" v-for="(item, index) in list" :key="index" @click="click"
			 :options="options" @content-click="empok">
...
<script>
	export default {
		...
		methods: {
			...
			//列表选中返回
			empok(index){
				uni.$emit("handClickEmp",{
					employeeName: this.list[index].employeeName,
					clockNo: this.list[index].clockNo
				});
				//然后返回
				uni.navigateBack();
			},
			...
			input(res) {
				...
				//发送ajax
				uni.request({
					url: uni.getStorageSync('basePath') + '/model/app/main/Help.Find.loadEmp.action',
					data: {
						filter: this.keyword
					},
					header: {
